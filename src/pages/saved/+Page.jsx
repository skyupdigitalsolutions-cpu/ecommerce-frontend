  ...
  if (ready && items.length === 0) {
    return (
      <section className="mx-auto max-w-[1400px] px-5 py-24 text-center sm:px-8 lg:px-12">
        ...
        <h1 className="mt-4 text-2xl font-bold text-[#0F1729]">No saved items yet</h1>
        <p className="mt-2 text-[#667085]">Tap the bookmark icon on any product or category to save it here.</p>
  ...
  return (
    <section className="w-full bg-white">
      <div className="mx-auto max-w-[1400px] px-5 py-10 sm:px-8 lg:px-12">
        ...
        <h1 className="text-2xl font-bold tracking-tight text-[#0F1729] sm:text-[32px]">Saved items</h1>
        ...
        <div className="mt-8 grid grid-cols-2 gap-4 sm:grid-cols-3 sm:gap-6 lg:grid-cols-5">
          {items.map((b) => (
            <div key={keyOf(b)} className="group relative overflow-hidden rounded-2xl border border-slate-100 bg-white p-3 transition hover:shadow-[0_22px_50px_-24px_rgba(15,23,41,0.4)]">
              <a href={b.href} className="block">
                ...
                {b.type === "product" && b.price != null && <p className="px-1 text-[13px] font-semibold text-[#0037CA]">₹{b.price}</p>}
                {b.type !== "product" && <p className="px-1 text-[12px] text-[#98A2B3]">Category</p>}
              </a>
              <button type="button" onClick={() => remove(keyOf(b))} aria-label="Remove" className="absolute right-3 top-3 grid h-8 w-8 place-items-center rounded-full bg-white/90 text-[#98A2B3] shadow-sm transition hover:text-[#DC2626]">
