...
import { ShoppingCart, ArrowRight, ChevronRightIcon } from "lucide-react";
import {
  getCategoryBySlug,
...
function Stars({ value = 0 }) {
  ...
  const row = () =>
    [0, 1, 2, 3, 4].map((i) => <Star key={i} className="h-4 w-4 flex-none" />);
  return (
    <span
...
/* per-category hero — full-width banner image with overlaid text */
    ...
    <div className="relative overflow-hidden ">
      ...
      <div className="relative flex min-h-[280px] items-center p-6 sm:min-h-[360px] sm:p-10 lg:min-h-[420px] lg:p-14">
        <div className="max-w-xl">
          <p className="text-[12px] font-bold uppercase tracking-widest text-[#93B4FF]">
            Custom Printing
          </p>
          <h1 className="mt-2 text-3xl font-bold leading-tight tracking-tight text-white sm:text-[44px] lg:text-[52px]">
            {category.name}
          </h1>
...
/* horizontal product rail — reused for Trending & New Launches */
  ...
  return (
    <div className="mx-12 mt-14">
      <div className="mb-6 flex items-end justify-between gap-4">
        <div>
          {eyebrow && (
            <p className="text-[12px] font-bold uppercase tracking-widest text-[#0037CA]">
              {eyebrow}
            </p>
          )}
          <h2 className="mt-1 text-xl font-bold tracking-tight text-[#0F1729] sm:text-2xl">
            {title}
          </h2>
        </div>
      </div>
      <div className="flex snap-x snap-mandatory gap-4 overflow-x-auto pb-2 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {products.map((p) => (
          <div
            key={p.id}
            className="shrink-0 basis-[70%] snap-start sm:basis-[42%] md:basis-[30%] lg:basis-[calc((100%-5rem)/6)]"
          >
            <ProductCard p={p} />
          </div>
        ))}
...
export default function Page() {
  ...
  const trending = getTrendingByCategory(slug, null, 6);
  const newLaunches = getNewByCategory(slug, null, 6);
  ...
  const visible =
    sub === "All" ? products : products.filter((p) => p.sub === sub);
  ...
  return (
    <section className="w-full bg-white">
      <div className="px-5 py-10 sm:px-8 sm:py-6 lg:px-0">
        ...
        <div
          id="products"
          className="mx-12 mt-10 flex items-center justify-between gap-4 scroll-mt-24"
        >
        ...
        {visible.length > 0 ? (
          ...
          <div className="mt-16 mx-12 flex flex-col items-center text-center">
            <p className="text-[17px] font-semibold text-[#0F1729]">
              No products in “{sub}” yet
            </p>
            <p className="mt-2 text-[#667085]">
              Try another filter or browse all.
            </p>
            <button
              type="button"
              onClick={() => setSub("All")}
