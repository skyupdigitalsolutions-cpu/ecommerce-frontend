    ...
    <article className="group flex h-full shrink-0 basis-[70%] snap-start flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white transition hover:shadow-[0_18px_45px_-24px_rgba(15,23,41,0.35)] sm:basis-[42%] md:basis-[30%] lg:basis-[calc((100%-5rem)/5)]">
      ...
      <div className="flex min-w-0 flex-1 flex-col p-4">
        ...
        <div className="mt-auto flex items-center gap-2 pt-4">
          ...
          <button type="button" aria-label={`Add ${p.title}`} className="grid h-11 w-11 flex-none place-items-center rounded-full border-2 border-slate-200 text-[#f87613] transition hover:border-black hover:bg-black hover:text-white"><ShoppingCart className="h-5 w-5" /></button>
        </div>
      </div>
    </article>
  );
...
export default function NewLaunches() {
  ...
  const items = [...products].sort((a, b) => (b.mrp - b.price) / b.mrp - (a.mrp - a.price) / a.mrp).slice(0, 10);
  if (items.length === 0) return null;
  return (
    <section className="w-full bg-white py-8 sm:py-10">
      <div className="mx-12 px-5 sm:px-8 lg:px-12">
        <div className="mb-6"><p className="text-[12px] font-bold uppercase tracking-widest text-[#2F6BFF]">Just dropped</p><h2 className="mt-1 text-2xl font-bold tracking-tight text-[#0F1729] sm:text-[38px]">New launches</h2></div>
        <div className="flex snap-x snap-mandatory gap-4 overflow-x-auto pb-2 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">{items.map((p) => <Card key={p.id} p={p} />)}</div>
      </div>
    </section>
  );
}